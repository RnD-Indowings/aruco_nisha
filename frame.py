...
def run(
    ...
    for i, det in enumerate(pred):  
        ...
        # Draw bounding boxes (YOLO's default behavior)
        ...
        # ADD CENTER DOT
        h, w, _ = im0.shape  # Get frame dimensions
        center = (w // 2, h // 2)  # Calculate center
        cv2.circle(im0, center, 5, (0, 0, 255), -1)  # Red dot at center
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if names[int(cls)] == 'person' :
                        ...
                        distance = math.sqrt((frame_center_x - 0) ** 2 + (frame_center_y - 0) ** 2)
                        if distance < min_distance:
                            min_distance = distance
                            #closest_person = (*xyxy, conf, cls)
                            print(f"person in {x1, y1, x2, y2} is the closest person to the center of the image")
                            print(f"width: {width}, height: {height}, frame: {frame}")
